...
func GetText(ctx context.Context, conn *db.SqlConnection, userID int64, id string) (*models.TextRecord, error) {
	const q = `
...
func ListTexts(ctx context.Context, conn *db.SqlConnection, userID int64) ([]*models.TextRecord, error) {
	const q = `
		SELECT user_id, id, text, meta, created_at, updated_at, is_deleted
		FROM texts
		WHERE user_id = $1
	...
	result := make([]*models.TextRecord, 0, len(rows))
	for _, r := range rows {
...
func scanTextRow(row []any) (*models.TextRecord, error) {
	if len(row) != 7 {
