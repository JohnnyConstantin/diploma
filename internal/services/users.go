...
// UsersService инкапсулирует операции над пользователями
...
func (s *UsersService) Register(ctx context.Context, login, password string) error {
	err := repo.CreateUser(ctx, s.db, login, password)
	if err != nil {
		if errors.Is(err, repo.ErrLoginExists) {
...
func (s *UsersService) Authenticate(ctx context.Context, login, password string) error {
	err := repo.AuthenticateUser(ctx, s.db, login, password)
	if err != nil {
...
func (s *UsersService) GetUserIDByLogin(ctx context.Context, login string) (int64, error) {
	id, err := repo.GetUserIDByLogin(ctx, s.db, login)
	if err != nil {
