...
func GetBinary(ctx context.Context, conn *db.SqlConnection, userID int64, id string) (*models.BinaryRecord, error) {
	const q = `
...
func ListBinaries(ctx context.Context, conn *db.SqlConnection, userID int64) ([]*models.BinaryRecord, error) {
	const q = `
		SELECT user_id, id, data, meta, created_at, updated_at, is_deleted
		FROM binaries
		WHERE user_id = $1
	...
	result := make([]*models.BinaryRecord, 0, len(rows))
	for _, r := range rows {
...
func scanBinaryRow(row []any) (*models.BinaryRecord, error) {
	if len(row) != 7 {
